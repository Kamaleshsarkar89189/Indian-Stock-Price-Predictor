...
# Make local and src modules importable
sys.path.append('.')
sys.path.append('src')
...
if run_button:
	try:
		...
		with st.spinner(f"Fetching data for {selected_stock_name}..."):
			df = fetch_stock_data(selected_stock, str(prediction_date), lookback_days)
		...
		if show_supervised:
			with tabs[tab_names.index("🤖 Supervised ML")]:
				...
				with st.spinner("Training models and generating predictions..."):
					...
					ensemble_pred = out["ensemble_prediction"]
					predictions_next = out["predictions"]
				...
				# To align predictions, re-run the split index here
				split_idx = int(len(df) * (1 - test_size))
				test_start_index = split_idx

				# We need y_test to overlay actual test targets; reuse from test_predictions length
				# As we don't carry y_test from the pipeline, skip plotting actuals unless provided by your integration.
				# Here we plot predicted series aligned on dates only.
				if test_predictions:
					# Build a pseudo y_test length from any one prediction array
					any_preds = next(iter(test_predictions.values()))
					if any_preds is not None:
						test_len = len(any_preds)
						# If target was next-step price, use the close series aligned to test window as proxy for trend
						if test_len > 0 and test_start_index + test_len <= len(df):
							...
							fig_overlay = plot_predictions_overlay(
								...
								y_test=None,  # Set to None if not available
